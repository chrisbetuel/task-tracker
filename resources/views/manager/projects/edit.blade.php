...
<div class="card mb-3">
    <div class="card-body">
        <form method="POST" action="{{ route('manager.projects.update', $project) }}">
            @csrf @method('PUT')
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="url" class="form-control @error('url') is-invalid @enderror" name="url" value="{{ old('url', $project->url) }}" placeholder="https://github.com/org/repo or https://example.com/project">
            ...
            @if($project->effectiveUrl())
            <div class="mb-3">
                <div class="alert alert-info small py-2 mb-0">
                    <i class="bi bi-info-circle"></i>
                    Effective URL: <a href="{{ $project->effectiveUrl() }}" target="_blank">{{ $project->effectiveUrl() }}</a>
                    @if(!$project->url && $project->parent)
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('status') is-invalid @enderror" name="status">
                    ...
                    <option value="accomplished" {{ old('status', $project->status) === 'accomplished' ? 'selected' : '' }}>Accomplished</option>
                </select>
                @error('status')<div class="invalid-feedback">{{ $message }}</div>@enderror
                <small class="text-muted">Leave empty for auto-calculation based on task progress.</small>
            </div>
            <div class="mb-3">
                <label for="parent_project_id" class="form-label">Parent Project</label>
                <select class="form-select @error('parent_project_id') is-invalid @enderror" name="parent_project_id">
                    <option value="">None (Top-Level Project)</option>
                    @foreach($projects as $proj)
                    <option value="{{ $proj->id }}" {{ old('parent_project_id', $project->parent_project_id) == $proj->id ? 'selected' : '' }}>{{ $proj->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <label class="form-label mb-0 fw-semibold">Add Sub-Projects</label>
                    <button type="button" class="btn btn-sm btn-outline-primary" onclick="addSubProject()">
                        <i class="bi bi-plus"></i> Add Sub-Project
                    </button>
                </div>
                <p class="text-muted small mb-3">Add new child projects nested under this project.</p>
            ...
            <div class="mb-3">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <label class="form-label mb-0 fw-semibold">Add Initial Tasks</label>
                    <button type="button" class="btn btn-sm btn-outline-success" onclick="addInitialTask()">
                        <i class="bi bi-plus"></i> Add Task
                    </button>
                </div>
                <p class="text-muted small mb-3">Create and optionally assign new tasks for this project.</p>
